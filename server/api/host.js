...
module.exports.list = (req, res) => {
	...
	mysql.query(sql, (err, rows, fields) => {
		...
		const host = rows.map((rows)=>{
			return {
				id: rows.user_id, on: rows.on ,name: rows.name,nickname: rows.nickname,sex: rows.sex,email: rows.email,
				password: rows.pw,phone: rows.phone, address: rows.address, photo: rows.photo,
				description: rows.description, language1: rows.language1, language2: rows.language2,
				language3: rows.language3, place:{
					formatted_address: rows.address, geometry: { location:{ lat:rows.host_latitude, lng:rows.host_longitude }}, name: rows.address
				}
			}
		})
